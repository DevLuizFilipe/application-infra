...
module "ecs_iam_staging" {
  source            = "./modules/iam/"
  iam_ecs_role_name = "iam_ecs_staging"
}
...
module "elb_staging" {
  source                               = "./modules/elb/"
  elb_name                             = "elb_application_staging"
  elb_type                             = "application"
  elb_subnets                          = [module.vpc_staging.subnet1_id, module.vpc_staging.subnet2_id]
...
module "application_service_staging" {
  ...
  ecs_service_name               = "application_staging"
  ecs_service_cluster            = module.ecs_staging.cluster_name
  ecs_service_task               = module.application_staging.task_arn
  ecs_service_count              = "1"
  ecs_service_subnets            = [module.vpc_staging.subnet1_id, module.vpc.subnet2_id]
  ecs_service_security_groups    = [module.vpc_staging.security_group_id]
  ecs_service_elb_arn            = module.elb_staging.elb_target_group_arn
  ecs_service_elb_container      = module.application_staging.task_container_name
  ecs_service_elb_container_port = module.application_staging.task_container_port
  depends_on                     = [module.vpc_staging, module.application_staging, module.elb_staging, module.ecs_staging]
}

#Cria um CDN para a aplicação
module "cdn_staging" {
  source                  = "./modules/cdn/"
  cdn_damin_name          = module.bucket_website_staging.domain_name
  cdn_origin_id           = module.bucket_website_staging.bucket_name
  cdn_enabled             = "true"
  cdn_ipv6                = "true"
  cdn_root_object         = "index.html"
  cdn_allowed_methods     = ["GET", "HEAD", "OPTIONS"]
  cdn_cached_methods      = ["GET", "HEAD", "OPTIONS"]
  cdn_cache_target_origin = module.bucket_website_staging.bucket_name
  ...
  depends_on              = [module.bucket_website_staging]
}

# #Cria um dominio e um record do tipo CNAME
# module "route53_staging" {
